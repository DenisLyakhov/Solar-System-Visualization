...
bool model3DS::readChunk(std::ifstream *modelFile,  int objectStart, int objectLength){
    ...
    while((modelFile->tellg() < (objectStart + objectLength)) && !modelFile->eof()){
		...
		if (offset == lastoffset) return false; // zablokowalo sie na odczycie dlugosci chunk
		lastoffset = offset;
...
{
	if (m_specialTransform == number) return; // juø jest  
	...
	if (!pregenetateList && !m_specialTransform)
		...
		for (int window=1;window < (m_parentModel->getStereo()?3:2);window++){
			...
			if (!m_specialTransform) {
				...
				glNewList(window==1?getListLeft():getListRight(),GL_COMPILE); //definicja zawartoùci listy("cube")
			}
			...
			if (!m_specialTransform) glEndList(); //koniec definicji zawartoùci listy
		}//for po window
}

// rysuje wszystkie siatki, pobiera jako parametr:
// - int: id tekstury: jeúli wiÍksze od 0 to wymusa automatyczne teksturowanie tπ teksturπ, jeúli 0 lub brak - przyjmuje teksturowanie z pliku
// - bool: centrowanie obiektu: jeúli true lub brak: za pozycjÍ obiektu przymuje jego geometryczny úrodek, jeúli false - przyjmje wspÛ≥rzÍdne z pliku
void model3DS::draw(int TextureID, bool centruj){
	...
	if (TextureID){ //gdy podano niezerowπ teksturÍ naleøy w≥aczyÊ automatyczne generowanie tekstur, w przeciwnym wypadku - koordynaty tekstury bazowej z modelu
	...
	if (texGenS == GL_TRUE) glEnable(GL_TEXTURE_GEN_S); else glDisable(GL_TEXTURE_GEN_S); // ustawienie poprzednich wartoúci
	if (texGenS == GL_TRUE) glEnable(GL_TEXTURE_GEN_T); else glDisable(GL_TEXTURE_GEN_T);
	m_stereo = false;
}

// ustala funkcjÍ specjalnπ (programowy vertex shader)
